...
export default async function TeamApprovals({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <AppShell role={ctx.profile.role} name={ctx.employee?.full_name_ar ?? ''}>
      <div className="space-y-6">
        <div className="card">
          <h2 className="mb-3 font-bold">{t('attendance.title')} — {t('approval.supervisorLevel')}</h2>
        ...
        <div className="card">
          <h2 className="mb-3 font-bold">{t('leave.title')} — {t('approval.supervisorLevel')}</h2>
          <div className="space-y-2">
            {leave.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3 text-sm">
                <div>
                  ...
                  {r.reason ? <p className="text-slate-400">{r.reason}</p> : null}
                </div>
