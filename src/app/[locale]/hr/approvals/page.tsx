...
import { getHrAttendanceQueue, getHrLeaveQueue } from '@/lib/data';
import { AppShell } from '@/components/AppShell';
import { StatusBadge, TamperBadges } from '@/components/StatusBadge';
import { ApprovalActions } from '@/components/ApprovalActions';

export default async function HrApprovals({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [att, leave] = await Promise.all([
    getHrAttendanceQueue(supabase) as Promise<any[]>,
    getHrLeaveQueue(supabase) as Promise<any[]>,
  ]);
  ...
  return (
    <AppShell role={ctx.profile.role} name={ctx.employee?.full_name_ar ?? ''}>
      <div className="space-y-6">
        <div className="card">
          <h2 className="mb-3 font-bold">{t('attendance.title')} — {t('approval.hrLevel')}</h2>
          <div className="space-y-2">
            {att.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3 text-sm">
                ...
                <ApprovalActions id={r.id} kind="attendance" level="hr" />
              </div>
            ))}
        ...
        <div className="card">
          <h2 className="mb-3 font-bold">{t('leave.title')} — {t('approval.hrLevel')}</h2>
          <div className="space-y-2">
            {leave.map((r) => (
              <div key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3 text-sm">
                <div>
                ...
                <ApprovalActions id={r.id} kind="leave" level="hr" />
              </div>
            ))}
